...
def perfect_square(player_score):
    for i in range(1, player_score + 1):
        if i**2 == player_score:
            return True
    return False


def next_perfect_square(player_score):
    i = player_score

    while not perfect_square(i) or i == player_score:
        i += 1
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            player_0_strat = strategy0(score0, score1)
            score0 = update(player_0_strat, score0, score1, dice)

        elif who == 1:
            player_1_strat = strategy1(score1, score0)
            score1 = update(player_1_strat, score1, score0, dice)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for player_score in range(0, goal):
        for opponent_score in range(0, goal):
            ...
            if strategy(player_score, opponent_score) != strategy(
                0, 0
            ):  # Here strat 0,0 is just a baseline we can compare to
                return False
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for num_rolls in range(1, 11):
        ...
        current_total = averaged_roll_dice(num_rolls, dice)

        if current_total > heighest_total:
            heighest_total, heighest_num = current_total, num_rolls
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    average_dice = make_averaged(roll_dice, 200)

    if square_update(0, score, opponent_score) - score >= average_dice(6):
        return 0
